...
**The matter versus manner axis (PC1).** The single ruler used throughout is the first principal component of the eight axis character space, computed by singular value decomposition of the `the internal reference table` (about 2.65 million domains), oriented so rigour plus depth is positive. Every script that needs the ruler rebuilds it the same way; the recipe is identical across `manip_analyse.py`, `cc_genre_state_fit.py`, `cc_state_fit_multi.py` and the reduction scripts.

**Where the data lives.** Public benchmark corpora and the internal crawl derivatives sit either in `the internal corpus store/<name>/` or in the internal schema of the internal Postgres (`the internal reference table`, `the internal Reddit corpus`, `the internal cross site corpus`, and the labelled tables named per experiment). The reference ruler table and the internal crawl tables are the internal hold set (Section 2).
...
**Held under review, reported not released.** Three assets rest on an internal web crawl and are not releasable: the 2.65 million domain character reference (`the internal reference table*`), the cross site pseudonymous authorship corpus (`the internal cross site corpus`, the coupling and equation of state substrate), and the reddit wide corpus with per author disposition (`the internal Reddit corpus`). Experiments resting on these report their numbers and commit their scripts, so they are reproducible in principle by anyone with a comparable crawl and the same apparatus, but the raw corpus is not distributed. Every such result says so in its own bounds.
...
**Tier two, HELD UNDER REVIEW, reported not released, an intellectual property and privacy hold rather than a security one.** The three internal crawl derivatives: the character reference of about 2.65 million domains (`the internal reference table*`), the cross site pseudonymous authorship corpus (`the internal cross site corpus`), and the reddit corpus with per author disposition (`the internal Reddit corpus`). Their results and scripts are public in tier one; the raw corpora are not distributed, because they rest on an internal crawl and carry pseudonymous personal data. Anyone with a comparable crawl and the tier one scripts can reproduce the findings. This tier is not government specific; it is simply not published.
